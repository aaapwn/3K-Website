...
const PlayerCard = ({
    ...
    return (
        <div className="flex flex-col gap-5 px-5 pb-10 rounded-xl bg-secondw drop-shadow-md align-top">
            ...
            <div>
                <h1 className="text-4xl font-bold text-firsto">{playerData?.university || "University Name"}</h1>
                <h1 className="text-5xl font-bold">{playerData?.nameThai || "John Doe"}</h1>
            </div>
            <div className="flex flex-row gap-1.5 items-center content-center">
                <Chip className="bg-firsto/10 text-firsto border-1 border-firsto rounded-full text-medium px-5 py-2"> {playerData?.sport || "Sport"} </Chip>
                <Chip className="bg-firsto/10 text-firsto border-1 border-firsto rounded-full text-medium px-5 py-2"> {playerData?.sportType || "Sport Type"} </Chip>
            </div>
        </div>
    );
}
