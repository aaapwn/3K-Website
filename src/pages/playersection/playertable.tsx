...
const PlayerTable = ({ playerID }: { playerID: string }) => {
    ...
    const renderCell = React.useCallback((match: Matches, column: React.Key) => {
        ...
        switch (column) {
            case "date":
                return <TableCell className=" text-secondb font-thin text-lg">{cellValue}</TableCell>;
            case "time":
                return <TableCell className=" text-secondb font-thin text-lg">{cellValue}</TableCell>;
            case "type":
                return <TableCell className=" text-secondb font-thin text-lg">
            ...
            case "detail":
                return <TableCell className=" text-secondb font-thin text-lg">
                    <Button onPress={onOpen} className="border-tertbg border-1 rounded-md text-tertbg bg-secondw px-5 py-2">
                        รายละเอียด
                    </Button>
                </TableCell>;
            default:
    ...
    return (
        <div className="flex flex-col gap-5 py-10 px-5 rounded-xl bg-secondw w-full h-full">
            ...
            <Table isStriped aria-label="table with dynamic content">
                <TableHeader columns={matchColumn} className="bg-firsto">
                    {(match) =>
                        <TableColumn className="bg-firsto text-secondw font-thin text-lg" key={match.key}>{match.label}</TableColumn>
                    }
                </TableHeader>
