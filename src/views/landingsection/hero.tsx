...
const HeroSection = ({ session }: { session: Session | null }) => {
  return (
    <WarpBackground
      className="bg-secondy/5 px-5 py-10"
      beamSize={2}
      beamDuration={4}
      gridColor="#7B818930"
    >
      <div className="grid grid-cols-1 md:grid-cols-2 max-w-7xl mx-auto my-0 gap-0">
        ...
        <div className="flex flex-col md:gap-5 gap-5 justify-center">
          <Chip className="bg-secondy p-5 text-base md:text-lg">
            รู้จักกับ 3K Games
          </Chip>
          <div className="flex-col gap-1">
            ...
            <h1 className="font-bold xl:text-3xl md:text-2xl text-xl text-firsto">
              ...
              สยามบรมราชกุมารี และกีฬา 3 พระจอมเกล้า ครั้งที่ 16
            </h1>
          </div>
          <h2 className="text-lg md:text-xl text-tertbg">
            ร่วมเป็นส่วนหนึ่งของประเพณีอันทรงเกียรติและการแข่งขันฟุตบอลที่ยิ่งใหญ่ที่สุดของ
            3 สถาบัน
          </h2>
          {session ? (
        ...
        <BlurFade className="w-full flex justify-end">
          <Image
            src={heroImage}
            alt="logo"
            className="object-cover xl:w-full h-full aspect-square"
          />
        </BlurFade>
      </div>
    </WarpBackground>
