...
export const getSportResultSummary = (
  sport: string
): Promise<SportResultSumary[]> => {
...
export const createAthleticResult = (
  token: string,
  data: CreataeAthleticResult
) => {
...
export const updateMedal = (token: string, data: UpdateMedal) => {
  return axiosClient.post("/result/medals", data, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
